...
static esp_err_t sensor_get_handler(httpd_req_t *req)
{
    ...
    float temp = 18.0 + (esp_random() % 10) / 10.0;
    
    // Format as JSON mensaje de jSON 
    char buffer[100];
    snprintf(buffer, sizeof(buffer), "{\"temperature\": %.1f}", temp);
    
    // Send Response envia respuesta HTTP
    httpd_resp_set_type(req, "application/json");
...
/* ---------------------------------------------------
 * ACTUATING CAPABILITY (POST /api/control) Sesive un post "llega mensaje de dashboard" 
 * --------------------------------------------------- */
static esp_err_t control_post_handler(httpd_req_t *req)
{
    ...
    if (total_len >= sizeof(buf)) {
        ...
        ESP_LOGE(TAG, "JSON demasiado grande");
    ...
    while (cur_len < total_len) {
        ...
        if (received <= 0) {
            ...
            ESP_LOGE(TAG, "Error recibiendo JSON");
